...
The hand-maintained `.d.ts` declarations (15,885 lines for XDR types alone, maintained via a separate `dts-stellar-sdk` tool) cannot express discriminated union narrowing. `switch(asset.switch().name)` gives `string`, not a narrowed literal type.

**ts-stellar-sdk solves all of these:**
```typescript
...
Rewrite `js-stellar-base` as a TypeScript-first, tree-shakeable library built on `ts-stellar-sdk`, while maintaining backward compatibility via a permanent compat layer for the js-xdr class-based API.
...
│   ├── generated/
│   │   ├── stellar.ts              # ts-stellar-sdk types (from xdrgen, tree-shakeable)
...
Two entry points:
- **`@stellar/stellar-base`** — new API. Tree-shakeable named exports. Plain ts-stellar-sdk objects. `Uint8Array` everywhere.
...
|---|---|---|
| `@stellar/js-xdr` (~50-80 KB) | `ts-stellar-sdk` (~8-10 KB, zero deps) | ~50 KB |
...
- `ts-stellar-sdk` — XDR codec engine
...
| `.toXDR(format?)` | Encodes via ts-stellar-sdk, converts to `Buffer`/base64/hex |
| `Type.fromXDR(input, format?)` | Parses input format, decodes via ts-stellar-sdk, wraps result |
...
function createCompatEnum(
  ...
  tsNameMap: Record<string, string>,      // legacyName → ts-stellar-sdk name
  tsCodec: XdrCodec<any>,
): CompatEnumClass;
...
4. Replace `@stellar/js-xdr` imports with `ts-stellar-sdk` imports
...
### Phase 1: ts-stellar-sdk completion
...
- [ ] Replace `@stellar/js-xdr` + `curr_generated.js` with `ts-stellar-sdk` + `stellar.ts`
...
|---|---|---|
| ts-stellar-sdk runtime | ~8 KB | Zero deps, minimal |
