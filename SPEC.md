# `ts-stellar-sdk` — TypeScript XDR Library Specification

## Context

Stellar needs a modern, TypeScript-first XDR library to replace the aging `@stellar/js-xdr` (runtime class-based, no TypeScript types). This spec defines `ts-stellar-sdk`: a generic RFC 4506 XDR codec library that will serve as the foundation for a new TypeScript Stellar SDK. The design is inspired by `rs-stellar-xdr` (Rust) but tailored to TypeScript idioms.
...
### 1.1 npm package: `ts-stellar-sdk`
...
A single Ruby file added to `stellar/xdrgen`, inheriting from `Xdrgen::Generators::Base`. Outputs a single `{namespace}_generated.ts` file. Generated code imports from `ts-stellar-sdk`.
...
// ---- Error handling ----
import { XdrError, XdrErrorCode } from 'ts-stellar-sdk';
try {
...
### Phase 1: Core Runtime (`ts-stellar-sdk`)
...
5. **Cross-implementation test**: Encode values with `ts-stellar-sdk`, decode with `rs-stellar-xdr` (and vice versa) to verify binary compatibility
