...
describe('building authorization entries', function () {
  ...
  [
    ...
    it(`signs the entry correctly (${methodName})`, async function () {
      const signedEntry = await StellarBase.authorizeEntry(authEntry, signer, 10);
  ...
  it('throws with a bad signature', function () {
    expect(
      StellarBase.authorizeEntry(
        authEntry,
        (_) => kp.sign(Buffer.from('bs')),
        10
      )
    ).to.eventually.be.rejectedWith(/signature doesn't match/i);
  });

  it('can build from scratch', async function () {
    await StellarBase.authorizeInvocation(kp, 10, authEntry.rootInvocation());
  });
});
