describe('StrKey', function () {
  let unencodedBuffer;
  let unencoded;
  let accountIdEncoded;
  let seedEncoded;
  beforeEach(function () {
    var keypair = StellarBase.Keypair.master(
      'Test SDF Network ; September 2015'
    );
    unencodedBuffer = keypair.rawPublicKey();
    unencoded = unencodedBuffer.toString();
    accountIdEncoded = keypair.publicKey();
    seedEncoded = StellarBase.StrKey.encodeEd25519SecretSeed(
      unencodedBuffer
    );
  });
  describe('#decodeCheck', function () {
    it('decodes correctly', function () {
      expect(
        StellarBase.StrKey.decodeEd25519PublicKey(accountIdEncoded)
      ).to.eql(unencodedBuffer);
      expect(
        StellarBase.StrKey.decodeEd25519SecretSeed(seedEncoded)
      ).to.eql(unencodedBuffer);
    });
  ...
  describe('#encodeCheck', function () {
    it('encodes a buffer correctly', function () {
      expect(
        StellarBase.StrKey.encodeEd25519PublicKey(unencodedBuffer)
      ).to.eql(accountIdEncoded);
      expect(
        StellarBase.StrKey.encodeEd25519PublicKey(unencodedBuffer)
      ).to.match(/^G/);
      expect(
        StellarBase.StrKey.encodeEd25519SecretSeed(unencodedBuffer)
      ).to.eql(seedEncoded);
      expect(
        StellarBase.StrKey.encodeEd25519SecretSeed(unencodedBuffer)
      ).to.match(/^S/);
      ...
      strkeyEncoded = StellarBase.StrKey.encodePreAuthTx(unencodedBuffer);
      expect(strkeyEncoded).to.match(/^T/);
      expect(StellarBase.StrKey.decodePreAuthTx(strkeyEncoded)).to.eql(
        unencodedBuffer
      );

      strkeyEncoded = StellarBase.StrKey.encodeSha256Hash(unencodedBuffer);
